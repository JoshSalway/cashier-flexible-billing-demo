    ...
    <script>
        ...
        function appendStep(logEl, step) {
            ...
            if (step.code) {
                ...
                const hiddenClass = allCodeVisible ? '' : 'hidden';
                codeHtml = `<div class="mt-1.5"><button onclick="document.getElementById('${codeId}').classList.toggle('hidden')" class="text-xs text-stripe hover:text-stripe-dark font-medium flex items-center gap-1"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5"/></svg>${allCodeVisible ? 'Hide Code' : 'Show Code'}</button><pre id="${codeId}" class="${hiddenClass} mt-1 text-xs bg-gray-900 text-green-300 rounded-lg p-3 overflow-x-auto"><code>${step.code.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</code></pre></div>`;
            }

            div.innerHTML = `<div class="flex items-start gap-2"><span class="${color} font-bold text-sm mt-0.5">${icon}</span><div class="flex-1"><div class="font-medium text-sm text-gray-900">${step.label}</div>${detail}${codeHtml}${linksHtml}${step.error ? `<div class="text-xs text-red-600 mt-1">${step.error}</div>` : ''}</div><span class="text-xs text-gray-400 shrink-0">${step.duration || ''}</span></div>`;
