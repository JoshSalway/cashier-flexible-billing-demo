    ...
    <script>
        ...
        function appendStep(logEl, step) {
            ...
            if (step.code) {
                ...
                codeHtml = `<div class="mt-1.5"><button onclick="document.getElementById('${codeId}').classList.toggle('hidden')" class="text-xs text-stripe hover:text-stripe-dark font-medium flex items-center gap-1"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5"/></svg>Show Code</button><pre id="${codeId}" class="hidden mt-1 text-xs bg-gray-900 text-green-300 rounded-lg p-3 overflow-x-auto"><code>${step.code.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</code></pre></div>`;
            }
